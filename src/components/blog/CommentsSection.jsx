...
export default function CommentsSection({
  ...
  return (
    <section className="mt-20 py-12 border-t border-slate-100">
      {/* Section Header */}
      <div className="flex items-center justify-between mb-10 dark:text-white dark:bg-slate-900">
      ...
      {/* 1. COMMENT FORM AREA */}
      {isAuthenticated ? (
        <div className="mb-16 bg-white dark:bg-slate-900 dark:border-slate-800 border border-slate-200 shadow-lg shadow-slate-200/50 rounded-lg overflow-hidden">
          <div className="bg-slate-50 dark:bg-slate-700 px-8 py-4 border-b border-slate-200 dark:border-slate-600 flex items-center justify-between">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest flex items-center">
              <ShieldCheck className="w-3 h-3 mr-1 text-[#6B46C1]" /> PUBLIC
              COMMENT ACTIVE
            </span>
          </div>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          ...
                          className="h-12 rounded-lg border-slate-200 focus:border-[#6B46C1] bg-slate-50 dark:bg-slate-900"
                          {...field}
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea
                          ...
                          className="rounded-lg border-slate-200 focus:border-[#6B46C1] bg-slate-50 dark:bg-slate-900 min-h-[120px]"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
            ...
            <div key={comment.id || index} className="group">
              <article className="flex items-start space-x-6">
                <Avatar className="h-12 w-12 border-2 border-white  shadow-md rounded-lg">
                  ...
                  <AvatarFallback className="bg-slate-100 dark:bg-slate-900 dark:border-slate-600 text-[#6B46C1] font-bold">
                    {comment.authorName?.substring(0, 1).toUpperCase() || "U"}
                  </AvatarFallback>
                </Avatar>

                <div className="flex-1 bg-white dark:bg-slate-900 dark:text-white p-6 border border-slate-100 group-hover:border-slate-200 transition-colors shadow-sm relative">
                  {/* Decorative accent */}

...
export default function BlogCard({ post }) {
  ...
  return (
    <Card className="group border border-border/50 shadow-lg dark:shadow-muted/5 rounded-lg overflow-hidden flex flex-col h-full bg-card text-card-foreground transition-all duration-300 hover:translate-y-[-8px]">
      ...
      <CardContent className="p-8 flex-grow">
        ...
        <CardTitle className="text-xl font-bold text-foreground mb-4 leading-snug tracking-tighter group-hover:text-[#6B46C1] transition-colors">
          <Link href={postHref}>{post.title}</Link>
        </CardTitle>

        <CardDescription className="text-muted-foreground text-sm line-clamp-3 leading-relaxed mb-6">
          {post.excerpt}
        </CardDescription>
      ...
      <CardFooter className="px-8 pb-8 pt-0 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Avatar className="h-8 w-8 border-2 border-background shadow-sm">
            ...
            <AvatarFallback className="bg-muted text-[#6B46C1] text-[10px] font-bold">

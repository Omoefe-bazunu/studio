...
export default function Header() {
  ...
  if (!mounted)
    return (
      <div className="h-16 w-full border-b bg-background" aria-hidden="true" />
    );

  return (
    <header
      role="banner" // FIX: Proper ARIA landmark
      className={cn(
    ...
    >
      <div className="container mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <Link
          ...
          aria-label="HIGH-ER ENTERPRISES Home"
        >
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                key={link.label}
                href={link.href}
                aria-current={isActive ? "page" : undefined} // FIX: Tell screen readers which page is active
                className={cn(
          ...
          <Sheet open={openSheet} onOpenChange={setOpenSheet}>
            <SheetTrigger asChild>
              <Button
                ...
                aria-label="Open mobile navigation menu" // FIX: Discernible name for icon button
              >
            ...
            >
              ...
              <nav
                className="flex-1 space-y-1 py-4 px-2"

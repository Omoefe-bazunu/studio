...
const blogPostFormSchema = z
  .object({
    ...
    category: z.custom<BlogCategoryType>(
      (val) => blogCategoriesData.some((cat) => cat.id === val),
      {
    ...
    imageSrc: z
      .string()
      .url("Invalid URL for existing image.")
      .optional()
      .or(z.literal("")),
    imageFile: z
    ...
    imageCreditText: z
      .string()
      .max(150, "Image credit text is too long.")
      .optional(),
    imageCreditLink: z
      .string()
      .url("Image credit link must be a valid URL.")
      .optional()
      .or(z.literal("")),
    excerpt: z
      .string()
    ...
    authorImageSrc: z
      .string()
      .url("Invalid URL for existing author image.")
      .optional()
      .or(z.literal("")),
    authorImageFile: z
      .instanceof(File, { message: "Please upload a valid author image file." })
      .refine(
        (file) => file.size <= MAX_FILE_SIZE_BYTES,
        `Max author image size is ${MAX_FILE_SIZE_MB}MB.`,
      )
      .refine(
        (file) => ACCEPTED_IMAGE_TYPES.includes(file.type),
        "Only .jpg, .jpeg, .png, and .webp formats are supported for author image.",
      )
      .nullable()
      .optional(),
  ...
  .refine((data) => data.imageFile || data.imageSrc, {
    message:
      "A main post image (either new upload or existing URL) is required.",
    path: ["imageFile"],
  });
  ...
  const editor = useEditor({
    ...
    extensions: [
      StarterKit.configure({
        // Disable StarterKit's heading so we use our own with levels config
        heading: false,
      }),
    ...
    editorProps: {
      attributes: {
        class:
          "min-h-[200px] p-3 focus:outline-none prose prose-sm max-w-none dark:prose-invert",
      },
    },
  });
                    ...
                    <FormItem>
                      ...
                      {currentImageSrc && !currentImageFile && (
                        <div className="my-2">
                          <Image
                            ...
                            data-ai-hint={
                              form.getValues("imageHint") || "blog post image"
                            }
                          />
                      ...
                      {currentImageFile && (
                        <p className="text-xs text-muted-foreground my-1 truncate">
                          New file selected: {currentImageFile.name}
                        </p>
                      )}
                      <FormControl>
                        <Input
                      ...
                      <FormDescription>
                        Max file size: {MAX_FILE_SIZE_MB}MB. Accepted: JPG, PNG,
                        WEBP.
                      </FormDescription>
                      <FormMessage>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder="Short summary of the post (max 200 chars)..."
                        {...field}
                        maxLength={200}
                      />
                    </FormControl>
              ...
              <FormField
                ...
                render={({ field, fieldState }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Supports bold, italic, headings (H1–H3), text alignment,
                      and embedded links. Content is saved as HTML.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
                  ...
                  render={({ field: { onChange, onBlur, name, ref } }) => (
                    <FormItem>
                      ...
                      {currentAuthorImageFile && (
                        <p className="text-xs text-muted-foreground my-1 truncate">
                          New file selected: {currentAuthorImageFile.name}
                        </p>
                      )}
                      <FormControl>
                        <Input

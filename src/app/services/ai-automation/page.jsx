...
const aiautomationProjectsRef = collection(db, "aiautomationProjects");

/** Subscribe to aiautomationProjects collection (real-time). Returns unsubscribe fn. */
const subscribeToaiautomationProjects = (callback) => {
  const q = query(aiautomationProjectsRef, orderBy("deliveryDate", "desc"));
...
/** Add a new aiautomation project (uploads screenshots first). */
const addaiautomationProject = async (data) => {
  const { urls, hints } = await processScreenshots(
    data.screenshots,
    `aiautomation_${Date.now()}`,
  );
  const { screenshots, ...rest } = data;
  await addDoc(aiautomationProjectsRef, {
    ...rest,
    screenshots: urls,
    imageHints: hints,
    category: "aiautomation Development",
...
/** Update an existing aiautomation project. */
const updateaiautomationProject = async (id, data) => {
  const { urls, hints } = await processScreenshots(data.screenshots, id);
  const { screenshots, ...rest } = data;
  await updateDoc(doc(db, "aiautomationProjects", id), {
    ...rest,
    screenshots: urls,
    imageHints: hints,
    sourceCodeUrl: data.sourceCodeUrl ?? "",
...
/** Delete a aiautomation project by ID. */
const deleteaiautomationProject = (id) =>
  deleteDoc(doc(db, "aiautomationProjects", id));

// ─── Constants ───────────────────────────────────────────────────────────────
const WHATSAPP_LINK = `https://wa.me/2349043970401?text=${encodeURIComponent(
  "Hello High-ER Enterprises, I'm interested in building an ai automation for my business.",
)}`;
      ...
      <CardFooter className="p-8 pt-0 flex flex-col sm:flex-row gap-3">
        ...
        >
          <Link href={project.liveUrl || "#"} target="_blank">
            VIEW PLATFORM <ExternalLink className="ml-2 h-4 w-4" />
          </Link>
        </Button>
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function aiautomationDevelopmentService() {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToaiautomationProjects((data) => {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      if (modal.edit) {
        await updateaiautomationProject(modal.edit.id, data);
        toast({ title: "Platform Updated" });
      } else {
        await addaiautomationProject(data);
        toast({ title: "New aiautomation Added" });
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteaiautomationProject(targetId);
      ...
      <section className="relative py-16 bg-[#0F0A1F] text-center overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-6">
          <div className="inline-flex items-center gap-2 px-4 py-1 rounded-full bg-white/5 border border-white/10 text-purple-200 text-xs mb-6">
            <Layers className="w-3.5 h-3.5 text-[#FF8C38]" aria-hidden="true" />
            <span>Business Automation</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-bold text-white leading-tight mb-6 max-w-3xl mx-auto">
            AI Automation <br />
            <span className="text-[#FF8C38]">For Your Buiness</span>
          </h1>
          <p className="mt-6 text-lg text-slate-400 max-w-xl mx-auto mb-10 leading-relaxed">
            Intelligent AI-powered workflows and tools that automate your
            invoicing, customer support, and repetitive tasks so you can focus
            on scaling your enterprise."
          </p>
          ...
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6 text-center md:text-left">
            <div className="w-full">
              ...
              <p className="text-slate-500 text-lg">
                A showcase of simple and complex automation projects devlivered
                to clients across different industries and geographical regions.
              </p>
            </div>
            {isAdmin && (
              <Button
              ...
              >
                <PlusCircle className="mr-2 h-5 w-5" /> Add aiautomation Project
              </Button>
            )}
          </div>
      ...
      >
        <AlertDialogContent className="rounded-[2.5rem]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove this aiautomation record.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
